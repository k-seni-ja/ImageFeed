...
final class ImagesListViewController: UIViewController {
    ...
    //MARK: - Properties
    private let photosName: [String] = Array(0..<20).map{"\($0)"} //массив с именами mock-фото
    ...
    override func viewDidLoad() {
        ...
        tableView.contentInset = UIEdgeInsets(top: 12, left: 0, bottom: 12, right: 0) // отступ контента от границ ячейки
    }
}
...
extension ImagesListViewController: UITableViewDataSource {
    ...
    func configCell(for cell: ImagesListCell, with indexPath: IndexPath) {
        guard let image = UIImage(named: photosName[indexPath.row]) else {
            print("⛔️ Не удалось загрузить картинку с таким именем \(photosName[indexPath.row])")
            return}
